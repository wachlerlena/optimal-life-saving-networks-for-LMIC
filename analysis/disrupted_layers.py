...
def _build_sample(pop, sample_size: int, base_size: int, sample_seed: int):
    """Sample `sample_size` rows that ALWAYS contain the `base_size`-row marker
    sample (same seed), so those base dots stay co-located with the clickable
    population markers while the rest add extra coverage.

    pandas' .sample(n) is not nested across different n, so we take the marker
    sample first and draw any extra rows from the remainder, then concatenate.
    """
    base = pop.sample(min(base_size, len(pop)), random_state=sample_seed)
    target = min(sample_size, len(pop))

    if target <= len(base):
        sample = base.iloc[:target]
    else:
        rest = pop.drop(index=base.index).sample(
            target - len(base), random_state=sample_seed
        )
        sample = pd.concat([base, rest])

    return sample.reset_index(drop=True)[["ID", "lat", "lon", "household_count"]]
...
def build_disruption_geojson(
    ...
    sample = _build_sample(pop, sample_size, base_size, sample_seed)
...
def add_disrupted_scenario_layers(
    ...
    _log(f"sampling {sample_size} rows from {pop_path.name} "
         f"(seed={sample_seed}, {base_size} coincident with markers)")
    pop = pd.read_pickle(pop_path)
    sample = _build_sample(pop, sample_size, base_size, sample_seed)
