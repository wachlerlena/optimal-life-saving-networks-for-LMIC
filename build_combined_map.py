...
from analysis.reinforcement_explorer import (
    EXPLORER_JSON,
    add_reinforcement_explorer,
)
...
(function() {{
    ...
    // Dedicated pane + canvas renderer for the scenario Δ dots. These are
    // created lazily inside the load handler below (NOT here at parse time):
    // this <script> is emitted into the page body BEFORE Folium's
    // `var {map_name} = L.map(...)` definition, so referencing {map_name} now
    // throws a ReferenceError that aborts the whole IIFE — which would silently
    // strip the overlayadd fetch handler and leave the dots unable to load.
    var scenarioRenderer = null;
    ...
    window.addEventListener('load', function() {{
        // The map variable is defined by the time 'load' fires. Create the
        // dedicated pane (z above the default overlay pane, below marker/tooltip
        // panes) so the scenario Δ dots always render ON TOP of the reinforcement
        // explorer's grid + mirror points, which live in the overlay pane and
        // would otherwise tint/overdraw them. Hospital markers (markerPane=600)
        // and tooltips (650) stay above the dots.
        if (!{map_name}.getPane('scenarioPane')) {{
            {map_name}.createPane('scenarioPane');
            {map_name}.getPane('scenarioPane').style.zIndex = 450;
        }}
        scenarioRenderer = L.canvas({{ pane: 'scenarioPane', padding: 0.5 }});
...
def _add_layer_control_click_toggle(m: folium.Map) -> None:
    """Give the (open-by-default) LayerControl a clickable 'Options' header that
    collapses/expands it on click — matching the click-to-toggle behaviour of the
    map's other legends, with no hover/auto-collapse."""
    js = """
<style>
.leaflet-control-layers .lc-header { font-weight:600; font-size:13px; cursor:pointer;
  display:flex; align-items:center; justify-content:space-between; padding:1px 2px 5px;
  user-select:none; }
.leaflet-control-layers .lc-header .lc-caret { color:#888; margin-left:12px; }
.leaflet-control-layers.lc-collapsed .leaflet-control-layers-list { display:none; }
</style>
<script>
(function() {
  function setup() {
    ...
    if (c.querySelector('.lc-header')) return;                  // already set up
    try { L.DomEvent.off(c, 'mouseenter mouseleave'); } catch (e) {}  // no hover
    var header = document.createElement('div');
    header.className = 'lc-header';
    header.innerHTML = '<span>Options</span><span class="lc-caret">&#9662;</span>';
    c.insertBefore(header, c.firstChild);
    c.classList.remove('lc-collapsed');                         // open by default
    header.addEventListener('click', function(e) {
      e.stopPropagation();
      var collapsed = c.classList.toggle('lc-collapsed');
      header.querySelector('.lc-caret').innerHTML = collapsed ? '&#9656;' : '&#9662;';
    });
...
def build_combined_map(
    ...
    # Interactive reinforcement explorer (K slider + coverage grid + spine).
    # Uses the precomputed JSON from `python -m analysis.reinforcement_explorer`.
    add_reinforcement_explorer(m, data_path=EXPLORER_JSON)
    ...
    if server_mode:
        ...
        disruption_data = build_disruption_geojson(
            ...
            sample_size=scenario_sample_size,
            base_size=max_marker_points,
        )
    ...
    # collapsed=False: the control renders fully (open by default, no hover
    # behaviour). _add_layer_control_click_toggle then adds an "Options" header
    # that collapses/expands it on click, like the map's other legends.
    folium.LayerControl(collapsed=False).add_to(m)
    _add_layer_control_click_toggle(m)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=1000,
        help=(
            "Number of Δ-travel scenario dots per layer. The first "
            "--max-marker-points of them stay co-located with the clickable "
            "population markers; the rest add extra coverage. Dots are cheap "
            "(no routing), so this can be much larger than --max-marker-points."
        ),
    )
...
def main() -> None:
    args = parse_args()
    # Force UTF-8 stdout/stderr so logging non-ASCII layer names (e.g. the "Δ"
    # in the disruption layers) never crashes when output is redirected to a
    # pipe or file, where Windows would otherwise fall back to cp1252.
    ...
    build_combined_map(
        ...
        scenario_sample_size=args.scenario_sample_size,
    )
