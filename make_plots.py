...
def fig_travel_time_by_scenario(tt_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    fig.suptitle("Travel time to nearest reachable stroke centre, by scenario", y=1.00)
...
def fig_catchment_shrinkage(catchment: pd.DataFrame, out_path: Path) -> None:
    ...
    fig.suptitle("Isochrone catchment shrinkage under each disruption scenario (national)", y=1.00)
...
def fig_regional_delta(tt_deltas: pd.DataFrame, out_path: Path) -> None:
    ...
    fig.suptitle("Regional disparity in travel-time degradation", y=1.00)
    fig.tight_layout(rect=(0, 0.03, 1, 0.98))
    fig.savefig(out_path)
    plt.close(fig)
    _log(f"saved {out_path.name}")
...
def fig_unreachable_share(tt_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    fig.suptitle("Share of population losing all connectivity, by tier and region", y=1.00)
    fig.tight_layout(rect=(0, 0, 1, 0.98))
    fig.savefig(out_path)
    plt.close(fig)
    _log(f"saved {out_path.name}")
...
def fig_centrality_lorenz(edge_centrality: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("Concentration of edge criticality (Lorenz curve)")
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.grid(alpha=0.25)
