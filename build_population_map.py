...
def build_top_k_hospital_popup_data(
    ...
    # ------------------------------------------------------------------
    # Pass 1: for every population row, pick its top-k nearest hospitals by
    # straight-line distance and build the (still empty) result skeleton.
    # We record which (population row, hospital slot) pairs actually need a
    # network route, grouped by the hospital's graph node, so that routing can
    # be done with ONE Dijkstra per hospital instead of one search per pair.
    # ------------------------------------------------------------------
    pop_records = []
    demand_by_hosp_node = defaultdict(list)
    ...
    for idx, row in out.iterrows():
        ...
        # argpartition finds the k nearest without fully sorting every hospital;
        # we then order those k by distance to keep nearest-first tie-breaking.
        if top_k < len(coord_dists):
            part = np.argpartition(coord_dists, top_k - 1)[:top_k]
        else:
            part = np.arange(len(coord_dists))
        top_idx = part[np.argsort(coord_dists[part])]

        entries = []  # serialized hospital result dicts (one per hospital slot)
        meta = []     # parallel routing metadata, NOT serialized
        ...
        rec = {
            ...
            "pop_lon": pop_lon,
            "pop_node": pop_node,
            "pop_graph_lat": pop_graph_lat,
            "pop_graph_lon": pop_graph_lon,
            "pop_offset_km": pop_offset_km,
            "entries": entries,
            "meta": meta,
        }
        rec_index = len(pop_records)
        ...
        for hosp_array_idx in top_idx:
            ...
            entries.append(entry)
            meta.append({
                "hosp_graph_lat": hosp_graph_lat,
                "hosp_graph_lon": hosp_graph_lon,
                "hosp_lat": hosp_lat,
                "hosp_lon": hosp_lon,
                "hosp_offset_km": hosp_offset_km,
            })
            ...
            if (
                ...
                demand_by_hosp_node[hosp_node].append((rec_index, slot_index))
            else:
                bad_node_count += 1
    ...
    # ------------------------------------------------------------------
    # Pass 2: one Dijkstra per unique hospital graph node. We use
    # dijkstra_predecessor_and_distance, which stores only a predecessor map and
    # distances (O(nodes) memory) rather than a full path list to every node
    # (which would be O(sum of path lengths) and can reach many GB). Paths are
    # then reconstructed ONLY for the population points that use this hospital.
    # Trees are computed one at a time and discarded to bound peak memory.
    # ------------------------------------------------------------------
    total_hosp_nodes = len(demand_by_hosp_node)
    print(f"Routing: computing shortest-path trees for {total_hosp_nodes} hospital nodes...")
    trees_done = 0

    for hosp_node, demands in demand_by_hosp_node.items():
        try:
            pred, time_dist = nx.dijkstra_predecessor_and_distance(
                graph, hosp_node, weight="travel_time_min"
            )
        except nx.NodeNotFound:
            bad_node_count += len(demands)
            continue

        for rec_index, slot_index in demands:
            rec = pop_records[rec_index]
            pop_node = rec["pop_node"]

            if pop_node not in time_dist:
                no_path_count += 1
                continue

            # Reconstruct the hospital -> population path by walking predecessors
            # back from the population node, then reverse to population -> hospital
            # so the drawn line and coordinate order match the original logic.
            path_h_to_p = []
            cur = pop_node
            while cur != hosp_node:
                path_h_to_p.append(cur)
                preds = pred.get(cur)
                if not preds:
                    break
                cur = preds[0]
            path_h_to_p.append(hosp_node)
            # path_h_to_p is population -> ... -> hospital (already the order we
            # want), since we walked from the population node toward the hospital.
            nodes = path_h_to_p

            network_time_min = float(time_dist[pop_node])
            network_km = 0.0
            for u, v in zip(nodes[:-1], nodes[1:]):
                network_km += float(graph[u][v]["length_km"])

            meta_slot = rec["meta"][slot_index]

            coords = []
            add_coord_if_valid(coords, rec["pop_lat"], rec["pop_lon"])
            add_coord_if_valid(coords, rec["pop_graph_lat"], rec["pop_graph_lon"])

            for node in nodes:
                lon, lat = node
                add_coord_if_valid(coords, lat, lon)

            add_coord_if_valid(coords, meta_slot["hosp_graph_lat"], meta_slot["hosp_graph_lon"])
            add_coord_if_valid(coords, meta_slot["hosp_lat"], meta_slot["hosp_lon"])

            entry = rec["entries"][slot_index]
            entry["distance_km"] = float(rec["pop_offset_km"]) + network_km + float(meta_slot["hosp_offset_km"])
            entry["estimated_travel_time_min"] = network_time_min
            entry["route_coords"] = coords
            entry["route_available"] = len(coords) >= 2

        trees_done += 1
        if trees_done % 10 == 0:
            print(f"  Completed {trees_done}/{total_hosp_nodes} hospital trees...")

        # Release this tree before computing the next one.
        del pred, time_dist

    # ------------------------------------------------------------------
    # Pass 3: sort each population row's hospitals by travel time (same ordering
    # as before), assign ranks, and serialize to top_k_hospitals_json.
    # ------------------------------------------------------------------
    for rec in pop_records:
        hospital_results = rec["entries"]
        ...
        out.at[rec["idx"], "top_k_hospitals_json"] = json.dumps(hospital_results)
        computed_rows += 1
    ...
    print(f"  Hospital shortest-path trees computed: {total_hosp_nodes}")
...
def build_population_map(
    ...
    m = folium.Map(
        ...
        zoom_start=6,
        tiles=None,
        # Render all vector layers (population points + the ~27k critical-edge
        # line segments) on a <canvas> instead of one SVG <path> per feature.
        # SVG bogs the browser down with tens of thousands of DOM nodes; canvas
        # draws them in one pass and keeps panning/zooming responsive.
        prefer_canvas=True,
    )
    # Basemap added explicitly so the LayerControl shows a friendly label
    # ("Base map") instead of the raw provider id ("cartodbpositron").
    folium.TileLayer("CartoDB positron", name="Base map", control=True).add_to(m)
    ...
    # In server_mode the heavy route geometry and heatmap data are NOT embedded
    # in the HTML (see inject_side_panel_and_routes / _add_lazy_heatmap). Write
    # them to a sibling data/ folder so the page can fetch them on demand.
    # NOTE: fetch() needs the page served over HTTP (e.g. `python -m http.server`)
    # — opening the HTML via file:// will block these requests.
    #
    # Skip this when a caller supplies _data_out (e.g. build_combined_map): that
    # caller collects route_data/heat_data and writes the data/ files itself, so
    # writing here too would duplicate the work and can collide on file locks.
    if server_mode and _data_out is None:
        ...
        # One small file per population point, fetched the first time it's clicked.
        route_files_written = 0
        for pop_id, routes in route_data.items():
            if not routes:
                continue
            route_file = os.path.join(data_dir, f"routes_{pop_id}.json")
            with open(route_file, "w", encoding="utf-8") as f:
                json.dump(routes, f)
            route_files_written += 1

        print(f"Server mode: wrote heatmap.json + {route_files_written} route files to {data_dir}")
        print("Serve the folder over HTTP to use the map, e.g.:")
        print(f"  python -m http.server --directory \"{out_dir}\"")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep the HTML small by writing route geometry and heatmap data to a "
            "sibling data/ folder instead of embedding them. Requires serving the "
            "output over HTTP (file:// will not work)."
        )
    )
