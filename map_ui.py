...
def inject_side_panel_and_routes(
    ...
    When server_mode is True the (large) per-point route geometry is NOT
    embedded in the HTML. Instead each point's routes are fetched on demand
    from data/routes_<popId>.json the first time that point is clicked.
    """
    map_name = map_obj.get_name()

    # In server_mode routes are fetched on demand — skip embedding the big blob.
    route_json = "{}" if server_mode else json.dumps(route_data)
    ...
    <script>
        ...
        function showHospitalRoute(popId, hospitalId) {{
            ...
            if (popRoutes !== undefined) {{
                // Data already in memory (standalone mode or previously fetched).
                _drawRoute(popRoutes[String(hospitalId)]);
                return;
            }}

            // Server mode: fetch this point's routes from a static JSON file.
            // window._ROUTES_URL_TPL is set by build_combined_map's lazy-layer JS.
            if (_routeFetchPending[String(popId)]) return;
            _routeFetchPending[String(popId)] = true;
            var routeUrl = (window._ROUTES_URL_TPL || 'data/routes_') + popId + '.json';
            fetch(routeUrl)
                .then(function(r) {{ return r.json(); }})
                .then(function(data) {{
                    routeData[String(popId)] = data;
                    _drawRoute(data[String(hospitalId)]);
                }})
                .catch(function(err) {{
                    console.warn('Failed to fetch route data for', popId, err);
                }});
        }}
